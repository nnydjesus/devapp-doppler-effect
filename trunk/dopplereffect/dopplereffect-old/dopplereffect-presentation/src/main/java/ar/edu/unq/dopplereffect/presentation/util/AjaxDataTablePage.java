...
public class AjaxDataTablePage<T extends Serializable> implements Serializable, ITable {
    ...
    private SearchPanelModel<T> search;
    ...
    public AjaxDataTablePage(final Panel parent, final String id, final String sortName, final SearchPanelModel<T> aSearch,
        ...
        this.setAjaxdataTable(new AjaxFallbackDefaultDataTable<T>(id, columns, new GenericSortableDataProvider<T>(id,
                this.getSearch(), sortName), SearchPanelModel.PAGE_SIZE));
    ...
    public SearchPanelModel<T> getSearch() {
        return search;
    }

    public void setSearch(final SearchPanelModel<T> search) {
        this.search = search;
    }
